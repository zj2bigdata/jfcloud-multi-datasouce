...
@Slf4j
@Component
public class MultiDuridDataSourceFactory implements MultiDatasourceFactory {
    @Override
...
//           Class<?> dataSourceClass = ClassLoader.getSystemClassLoader().loadClass(dataSourceProperties.getDataSourceClass());
           Class<?> dataSourceClass = Class.forName(dataSourceProperties.getDataSourceClass());
           dataSource = dataSourceClass.newInstance();
           // 获取属性并设置
           Field driverClassName = dataSourceClass.getSuperclass().getDeclaredField("driverClass");
           ...
           Method setJdbcUrl = dataSourceClass.getSuperclass().getDeclaredMethod("setUrl", jdbcUrl.getType());
    ...
    public boolean match(String dataSourceClass) {
        return StrUtil.equals(dataSourceClass,"com.alibaba.druid.pool.DruidDataSource");
    }
}
